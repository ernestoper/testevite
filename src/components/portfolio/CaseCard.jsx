...
function CaseCard({data,index}){
    ...
    return(
        ...
        onMouseEnter={()=>{
            ...
            img.classList.add('hover:object-scale-down')
        }} 
        onMouseLeave={()=>{
            const title_element = document.getElementById(index)
            title_element.classList.remove('text-orange-500')
            const img = document.getElementById(data.id)
            img.classList.remove('hover:object-scale-down')
        }} 
        
        className="flex flex-col overflow-hidden  rounded-lg shadow-2xl">
                <div className="flex-shrink-0">
                  <img id={data.id} className="h-full w-full   object-cover" src={data.imageUrl} alt="" />
                </div>
